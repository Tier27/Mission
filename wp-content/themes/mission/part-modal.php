    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="well" id="manage-reservation-target">
          <form class="form-horizontal reservations" method="post" action="<?php echo bloginfo('wpurl'); ?>/payment">
            <fieldset>
		...
		<div class="inline">
                ...
                <div class="controls">
		  <?php $date = new rvDate( date('Y-m-d', strtotime(' + 1 day ')) ); ?>
		<div class="dropdown">
                  <select type="text" class="input-sm" id="book-reservation-start" name="start">
		<?php array_pop( $date->hours ); foreach ( $date->hours as $hour ) { if ( $date->check_hour( $hour ) ) { 
			$max = ( $hour == 15 || $hour == 16 ) ? 4 : 3;	
		?>
                    <option value=<?php echo $hour; ?> data-lanes=<?php echo $lanes = min( $max-$date->weblanes[$hour], $date->count_hour( $hour ) ); ?> data-price=<?php echo $date->settings->get_pricing( $date->day, $hour ); ?>><?php echo rvFormat::military_hour( $hour ); ?> <?php //echo $lanes; ?></option>
		<?php } } ?>
                  </select>
<!--
...
-->
                ...
                <div class="controls">
                  <select type="text" class="input-sm" name="hours" id="book-reservation-hours-count" name="lanes" data-original-title="Lanes">
                    <option value=1>1</option>
                    <option value=2>2</option>
                  </select>
                </div> 
              ...
              <!--<form role="form" class="reservations">-->
                <div class="row">
                  ...
                  <div class="col-xs-8">
                    <input type="text" class="form-control" name="name[first]" id="book-reservation-first-name" placeholder="First name">
                  </div>
                </div>
                <div class="row">
                  <label class="col-xs-4" for="name">Last name:*</label>
                  <div class="col-xs-8">
                    <input type="text" class="form-control" name="name[last]" id="book-reservation-last-name" placeholder="Last name">
                  </div>
                </div>
                <div class="row">
