	...
	// GENERATE TIME for event
		public function generate_time_($DATE_start_val='', $DATE_end_val='', $pmv, $evcal_lang_allday, $focus_month_beg_range='', $FOCUS_month_int=''){
			...
			// INITIAL variables
				...
				$_current_date = date('j', $focus_month_beg_range );
		...
		foreach($event_list_array as $event):
			...
			*/
			...
			// EVENT FEATURES IMAGE
				...
				if($img_id!=''){				
					...
					$_eventcard['ftimage'] = array(
						...
						'fullheight'=> $this->evopt1['evo_ftimg_fullheight'],
					);	
			...
			$href = (!empty($ev_vals['evcal_exlink']) && $exlink_option!='1' )? 
				'data-exlk="1" href="'.$ev_vals['evcal_exlink'][0].'?l='.$__shortC_arg['lang'].'"'
				:'data-exlk="0"';
			...
			$eventtop_html=$eventop_fields_='';
				...
				"<p class='ev_ftImg' style='background-image:url(".$img_thumb_src[0].")'></p>":null;
				...
				$eventtop_html[]="<p class='evcal_cblock' bgcolor='".$event_color."' smon='".$DATE_start_val['F']."' syr='".$DATE_start_val['Y']."'><em class='evo_date' >".$___day_name.$_event_date_HTML['html_date'].'</em>';
