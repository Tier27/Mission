...
class EventON {
	...
	/** Register/queue frontend scripts. */
		...
		public function register_evo_dynamic_styles(){
			...
			if(!empty($opt['evcal_style_header']) && $opt['evcal_style_header'] =='no' || empty($opt['evcal_style_header'])){
				if(is_multisite()) {
		...
		public function load_dynamic_evo_styles(){
			$opt= get_option('evcal_options_evcal_1');
			if(!empty($opt['evcal_style_header']) && $opt['evcal_style_header'] =='yes'){
				//ob_start();
				//include('assets/css/dynamic_styles.php');
